        ...
        *userDatasetPath = "/ebrc/workspaces/users/*userId/datasets/$dataId";
  	    ...
  	    *datasetSize = getCollectionSize(*stagingDatasetPath);
  	    if(*datasetSize + *collectionSize > *defaultQuota) {
  	        cleanup(*fileDir, *fileName, *stagingDatasetPath)
  	        acUserIssue(trimr(*fileName,"."), *message);
  	        msiGoodFailure;
  	    }
...
Jenkins may be offline or the listener job maybe disabled.  A later scheduled run should pick up the install event.");
        ...
        # At this point the tarball and the staging collection are no longer needed.  The export is successful at
        # this point.
	      cleanup(*fileDir, *fileName, *stagingDatasetPath)
    ...
    *error = "";
    {
        ...
        *event = *eventContent.event;
        ...
        # Make a RESTful call to Jenkins to process the contents of the events collection.
	      writeLine("serverLog", "Triggering delivery of events to Jenkins.");
	      acTriggerEvent(false);
    } ::: {
	      acSystemIssue('Internal', "IRODS acPostProcForPut/acPostProcForDelete", *warning, *error);
	  }
}
    ...
    *error = "";
    {
        ...
        *event = *eventContent.event;
	      *eventFileName = *eventContent.event_file_name;
	      ...
	      acSystemIssue('Internal', "IRODS acPreprocForRmColl", *warning, *error);
    }
}
...
acTriggerEvent(*export) {
    ...
    *argv = str(*irodsIdValue);
	  ...
	  # If the command fails, percolate the error for an export only.
	  if(*export) {
	    ...
	    *status = errorcode(msiExecCmd("executeJobFile.py", *argv, "null", "null", "null", *jobOutput));
	    if(*status != 0) {
	      writeLine("serverLog", "Trigger failure");
	      acSystemIssue('Internal', "IRODS Non install", 'Trigger failure - Jenkins listener may be offline', 'warning')
	    }
	  }
...
# Convenience method to remove leftover tarball and staging collection.  If either or both
# activities fail, they fail silently insofar as the user and the notification list is concerned.
cleanup(*tarballDir, *tarballName, *stagingDatasetPath) = {

    # Remove the tarball
    *dataObjectExists = checkForDataObjectExistence(*tarballDir, *tarballName);
    if(*dataObjectExists) {
        *tarballFile = *tarballDir ++ "/" ++ *tarballName;
        writeLine("serverLog", "Cleanup - Removing *tarballFile tarball.");
        errorcode(msiDataObjUnlink("objPath=*tarballFile++++forceFlag=",*actionStatus));
    }
